...
| `.node-version` | 固定本地与 CI 使用的 Node.js 版本 |
...
**要求**：
- Node.js 使用 22.22.2；
- pnpm 继续使用根 `package.json` 已声明的 10.33.2；
- 不升级 Node 24；
- 不修改根 `engines.node` 的最低版本约束；
- 本地开发者可通过支持 `.node-version` 的版本管理工具读取；
- GitHub Actions 必须读取该文件；
- quality 和 tauri-build 两个 Job 使用同一个 Node 版本来源。
...
**要求**：
- 使用 jsdom；
- 显式使用非不透明测试来源（`http://localhost/`）；
- 确保 `localStorage` 可稳定使用；
- 为 Hash Router 和地址相关测试提供确定性基础 URL；
- 不启用 Vitest globals；
...
`AppRouting.test.tsx` 会渲染真实 `App`，真实 App 会执行 `useThemeSync()`，因此测试环境必须提供 `window.matchMedia`。

setup 负责：
1. 导入 jest-dom Vitest matchers；
2. Testing Library cleanup；
3. 安装确定性的 `window.matchMedia` 实现；
4. 每个测试后清理 localStorage；
5. 清理根元素上的主题状态。

matchMedia 实现必须满足 `MediaQueryList` 使用需要，至少包含：
...
- 不使用 `@ts-ignore`；
- 不使用 `@ts-expect-error`；
- `addEventListener` 和 `removeEventListener` 均存在；
- 同一个 MediaQueryList 对象可以被 `useThemeSync` 注册和移除监听；
- 不访问真实系统主题；
- 不在单个测试中重复安装全局实现；
- 不吞掉 React 错误。
...
- 创建独立 QueryClient；
- `retry: false`；
- 不共享 QueryClient cache；
- 测试完成后释放或清理 QueryClient；
- 使用 `createMemoryRouter`；
- 不使用生产 Hash Router 单例；
- 不直接裸渲染 App。
...
- 不直接调用 Tauri `invoke`；
- 首页路由测试等待成功页面出现；
- 验证 `DevForge` 或版本号，而不是只断言 loading。
...
通过真实 App + AppLayout + Outlet 渲染以上页面，证明 App 没有被裸渲染。

**不得**：
- 复制生产 Hash Router 单例；
- 使用 `window.location.hash` 驱动测试；
- 测试 CSS 像素值；
- 使用快照替代语义断言；
- 使用 sleep。
...
`AppRouting.test.tsx` 只证明路由、Provider 和页面集成，不重复完整错误重试测试。

两个测试文件都必须：
- 创建独立 QueryClient；
- 不共享 cache；
- 不调用真实 Tauri；
- 使用 Testing Library 的异步查询；
- 不依赖测试执行顺序。
...
AppRouting tests do not call real Tauri IPC
HomePage tests do not share QueryClient cache
```
